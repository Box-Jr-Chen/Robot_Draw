...
class OPENCV_Analyze:
    ...
    def Anayzie_Cam_Img_sketch(self,image,num_clusters,contrast,brightness):
        ...
        try: 
            ...
            if Color_Firt[2] <=85:# 如果是黑色
                Color_Pre =  f'黑色' 
            elif Color_Firt[2] >85 and Color_Firt[2] <=130:# 如果是黑色
                Color_Pre =  f'灰色'
            elif Color_Firt[1] <=100:# 如果是白色
                Color_Pre =  f'白色'
            elif scale >=0.5 and  Color_Firt[1] <=100:# 如果是白色
                Color_Pre =  f'白色' 
            else:
                hm_a =0
                sm_a =0
                ...
                if rangeabs <=15 :
                    ...
                    Color_Pre = self.Anayzie_Color((hm_a,sm_a,vm_a))  
    ...
    def Anayzies_Color_block(self,img_block):
            ...
            color_hsv_array = [x for x in color_hsv_array if x['color'] != [0,0,0]] # 移除所有黑色

            
            for c in color_hsv_array :
                (hc, sc, vc)  = colorsys.rgb_to_hsv(c['color'][2]/255, c['color'][1]/255,  c['color'][0]/255)
            ...
            for predict, values in result_dict.items():
                ...
                result_list.append({'predict': predict, 'percent_sum': percent_sum, 'colors': colors})
    ...
    def Set_img_ori_draft(self, path: str) :
        ...
        if os.path.isfile(self.path_image_draft):
            ...
            #用於顯示
            self.common.picPc_ori_draft_block_dis =  copy.deepcopy(self.common.picPc_ori_draft)
            # 上下顛倒圖像
            ...
            self.common.picPc_ori_draft_bone,paths_draft = self.img_skeletonize(imgBin,False)
    ...
    def Set_img_ori_colorful(self, path: str) :
        ...
        if os.path.isfile(self.path_image_colorful):
            ...
            self.common.picPc_ori_colorful = cv2.cvtColor(self.common.picPc_ori_colorful, cv2.COLOR_BGR2RGB)
            ...
            for block in  self.common.blocks_colorful_countor :
                ...
                self.common.block_colorful_colors_info.append(self.Anayzies_Color_block(color_block)) # 顏色資訊
    ...
    def fun_draft_seleted_block(self,num):
        ...
        self.common.picPc_ori_draft_block_dis =  copy.deepcopy(self.common.picPc_ori_draft)
        self.draw_line_points(False,self.common.picPc_ori_draft_block_dis,self.common.blocks_colorful_countor[num],(0,0,255),(0,0,255),5,1)
    ...
    def fun_colorful_seleted_block(self,num):
        ...
        self.common.picPc_ori_colorful_dis =  copy.deepcopy(self.common.picPc_ori_colorful)
        self.draw_line_points(False,self.common.picPc_ori_colorful_dis,self.common.blocks_colorful_countor[num],(0,0,255),(0,0,255),1,1)
        ...
        for color_l in blocks_path:
            for coutur_l in color_l:
                for path_num in range(0, len(coutur_l)):
                        self.draw_line_points(False,self.common.picPc_ori_colorful_dis,coutur_l[path_num],(255,0,0),(0,255,255),2,1)
    ...
    def fun_colorful_seleted_color(self):
        ...
        if is_green_process:
                predict =self.common.block_colorful_colors_info[self.common.select_block_num][self.common.select_block_color_num]['predict']
                if predict =='綠色':
                    self.common.picPc_ori_colorful_dis_mask = mask
                elif predict =='春綠色':
                    img = self.common.picPc_ori_colorful_blocks[self.common.select_block_num]
                    self.common.picPc_ori_colorful_dis_mask = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
                elif predict =='黃綠色':
                    self.common.picPc_ori_colorful_dis_mask = self.get_color_mask(self.common.select_block_num,self.common.select_block_color_num)
...
#-----------------------------判斷
    ...
    def is_green_process(self,block_num):
        ...
        if len(block_colors) ==3:
            ...
            if is_green == True and is_spring_green == True and is_chartreuse_green == True:
    ...
    def draw_line_points(self,need_inner,img,block,color_p,color_l,size_p,size_l):
            for pnum in range(0,len(block)):
                ...
                if pnum < len(block) -1 :
                    p_next = block[pnum+1]
                    cv2.line(img, (int(p[0]),int(p[1])),(int(p_next[0]),int(p_next[1])), color_l, size_l)
    ...
    def img_skeletonize(self,imgBin,is_in_contour):
        ...
        # 畫出路徑
        for line in paths_get :
            self.draw_line_points(False,skeleton_rgb,line,(0,0,255),(0,255,0),2,1)
    ...
    def get_image_contours(self,image,need_out):
        ...
        for b, cnt in enumerate(contours):
            ...
            if need_out == False:
                #去掉父階層
                if hierarchy[0, b, 3] != -1:
                    approx = cv2.approxPolyDP(cnt, 0.0013*cv2.arcLength(cnt, True), True)
                    blocks.append(approx)
            else :
    ...
    def img_color_check(self,block):
        ...
        cropped_img = self.common.picPc_ori_colorful[topmost:bottommost, leftmost:rightmost]
    ...
    def get_color_mask(self,block_num,color_num):
        ...
        predict = self.common.block_colorful_colors_info[block_num][color_num]['predict']
    ...
    def get_poly_paths(self,contour):
        ...
        for line_pan_num in range(0,len(lines_Ori)):
            ...
            for pnum in range(0,len(contour)):
                ...
                point_is_exit,point_cross = self.cross_point(lines_Ori[line_pan_num],[p1,p2])
        ...
        try:
            if self.cap_color is not None :
                ...
                if  ret:
                    ...
                    y_start = int(height_f2) - int(self.common.get_ColorPaletteSize_start[1])
                    y_height = int(height_f2) - (int(self.common.get_ColorPaletteSize_start[1]) +int(self.common.get_ColorPaletteSize[1]))
                    ...
                    y_start = int(height_f2) - int(self.common.get_ColorRealSize_start[1])
                    y_height =  int(height_f2) - (int(self.common.get_ColorRealSize[1]) +  int(self.common.get_ColorRealSize_start[1]))
                    ...
                    # 執行旋轉操作
                    frame_real = cv2.warpAffine(frame_real, M, (frame_real.shape[1], frame_real.shape[0]))
                    ...
                    for i in range(0,len(self.common.Color_Cells)):
                        ...
                        r_x_start = int(self.common.PalCell_offset[0])*x_row+int(self.common.PalCell_start[0]) + x_row *10 + x_row * int(self.common.PalCell[0])
                        r_y_start = int(self.common.PalCell_offset[1])*y_row+int(self.common.PalCell_start[1]) + y_row *10 + y_row * int(self.common.PalCell[1])
                        r_x_end   = int(self.common.PalCell_offset[0])*x_row+int(self.common.PalCell_start[0]) + (x_row+1) *10 + (x_row+1) * int(self.common.PalCell[0])
                        r_y_end   = int(self.common.PalCell_offset[1])*y_row+int(self.common.PalCell_start[1]) + (y_row+1) *10 + (y_row+1) * int(self.common.PalCell[1])
                    
                        #新增調色盤上的區塊內顏色
                        frame_color =  frame_plant[r_y_start:r_y_end,r_x_start:r_x_end] 
                        Anayzie_image,HSV_Main,Color_Pre = self.Anayzie_Cam_Img_sketch(frame_color,2,self.common.pal_contrast,self.common.pal_brightness)
                        
                        
                        frame_color_image = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(Anayzie_image))
                    ...
                    for i in range(0,len(self.common.Pal_main_start)):
                        ...
                        r_x_end   = int(self.common.Pal_main_start[i][0])+int(self.common.Pal_main[0])
                        r_y_end   = int(self.common.Pal_main_start[i][1])+int(self.common.Pal_main[1])
                        
                        frame_color =  frame_plant[r_y_start:r_y_end,r_x_start:r_x_end] 
                        Anayzie_image,HSV_Main,Color_Pre = self.Anayzie_Cam_Img_sketch(frame_color,2,self.common.main_color_contrast,self.common.main_color_brightness)
                        frame_color_image = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(Anayzie_image))
                        ...
                        cv2.rectangle(frame_plant, (r_x_start, r_y_start), 
                                        (r_x_end , r_y_end),
                                        (0, 100, 255), 1, cv2.LINE_AA)

                    image_plant =  PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(frame_plant))
                    
                    self.common.image_pic_real =   PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(frame_real))
