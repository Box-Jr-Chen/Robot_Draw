...
class RobotCumtomerUI:
    ...
    def get_pic_ori_colorful(self):
        ...
        if filename != "":
            ...
            if self.common.select_block_num != -1:
                self.common.OPENCV_Analyze.fun_colorful_seleted_block(self.common.select_block_num)
            ...
            self.picPc_color_canvas.create_image(0, 0, anchor=tk.NW, image=self.img_colorful)
    ...
    def compare_pic_real(self):
        word_label = tk.Label(self.tab_PicReal, text='電腦圖片 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=1000, y=5)
        
        self.picReal_campare_canvas = tk.Canvas(self.tab_PicReal, width=self.width_picPc_ori,
                                height=self.width_picPc_ori,bg='gray',highlightthickness=1, highlightbackground="black")
        self.picReal_campare_canvas.place(x=1000, y=30)

        if self.img_colorful is not None :
          self.picReal_campare_canvas.create_image(0, 0, anchor=tk.NW, image=self.img_colorful)
    ...
    def get_robot_main_test(self):
         ...
         self.common.RobotBehavior.fun_robot_behavior[9](self.robot_draw_test_x,self.robot_draw_test_y)
    ...
    def show_tooltip_listbox(self,event):
        ...
        if index < len(self.common.block_colorful_colors_info[self.common.select_block_num]):
            for c_num in range(0,len(self.common.block_colorful_colors_info[self.common.select_block_num])):
                if index == c_num :
                    self.colorful_box_blocktext = ','.join(map(str,self.common.block_colorful_colors_info[self.common.select_block_num][c_num]['colors']))
        else :
             self.colorful_box_blocktext = ''
    ...
    def on_select_block(self,event):
        ...
        if isinstance(self.common.picPc_ori_colorful, np.ndarray):
          ...
          # block
          im = Image.fromarray(self.common.picPc_ori_colorful_blocks[self.common.select_block_num])
          ...
          self.picPc_color_block_canvas.create_image(0, 0, anchor=tk.NW, image=self.img_colorful_block)
          
          self.colorful_box_blocks.delete(0, tk.END)
          
          for c in self.common.block_colorful_colors_info[self.common.select_block_num]:
          ...
          ToolTip(self.colorful_box_blocks, self.get_colorful_box_blocktext, follow=True,delay=0)
    ...
    def on_select_color(self,event):
        ...
        self.common.select_block_color_num = int(self.colorful_box_blocks.curselection()[0])
    ...
    def fun_create_paths_Test_inRobotControl(self):
        ...
        word_label = tk.Label(tab_RobotControl, text='線稿測試 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=10, y=470)

        self.robot_draft_test_btn  = tk.Button(tab_RobotControl, text=f'線稿動作', width=36, command=self.common.RobotBehavior.fun_robot_behavior[10])
        self.robot_draft_test_btn.place(x=100, y=470)
        
        word_label = tk.Label(tab_RobotControl, text='彩色測試 :',font=("Microsoft JhengHei", 12))
        ...
        for i in range(0,len(self.common.picPc_ori_colorful_blocks)):
            ...
            self.robot_draft_test_btn  = tk.Button(tab_RobotControl, text=f'區塊_{i+1}', width=14, command=partial(self.common.RobotBehavior.fun_robot_behavior[11], i))
            self.robot_draft_test_btn.place(x=100+ x_p *120, y=520 +y_p * 40)
        
        robot_pal_main_color_btn = tk.Button(tab_RobotControl, text=f'全區塊調色畫圖', width=36, command=self.common.RobotBehavior.fun_robot_behavior[13])
    ...
    def fun_tab_RobotControl(self):
        ...
        # 動作初始
        word_label = tk.Label(tab_RobotControl, text='手臂動作 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=10, y=10)
        # 手臂初始
        self.robot_init_btn = tk.Button(tab_RobotControl, text='手臂初始', width=36, command=self.common.RobotBehavior.fun_robot_behavior[0])
        self.robot_init_btn.place(x=100, y=10)
        #開始繪製(start)
        self.robot_draw_start_btn = tk.Button(tab_RobotControl, text='開始繪製', width=36, command=self.common.RobotBehavior.fun_robot_behavior[1])
        self.robot_draw_start_btn.place(x=400, y=10)
        #重啟繪製(restart)
        self.robot_draw_restart_btn = tk.Button(tab_RobotControl, text='重啟繪製', width=36, command=self.common.RobotBehavior.fun_robot_behavior[2])
        self.robot_draw_restart_btn.place(x=700, y=10)
        #暫停繪製(stop)
        self.robot_draw_stop_btn = tk.Button(tab_RobotControl, text='暫停繪製', width=36, command=self.common.RobotBehavior.fun_robot_behavior[3])
        self.robot_draw_stop_btn.place(x=1000, y=10)
        #停止繪製(pause)
        self.robot_draw_pause_btn = tk.Button(tab_RobotControl, text='停止繪製', width=36, command=self.common.RobotBehavior.fun_robot_behavior[4])
        self.robot_draw_pause_btn.place(x=1300, y=10)
        
        
        word_label = tk.Label(tab_RobotControl, text='動作測試 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=10, y=60)
        # 沾水
        self.robot_dig_water_btn = tk.Button(tab_RobotControl, text='沾水', width=36, command=self.common.RobotBehavior.fun_robot_behavior[5])
        self.robot_dig_water_btn.place(x=100, y=60)
        # 沾衛生紙
        ...
        # 沾主畫區
        self.robot_draw_block_btn = tk.Button(tab_RobotControl, text='主畫區測試', width=36, command=self.get_robot_main_test)
        self.robot_draw_block_btn.place(x=100, y=160)

        word_label = tk.Label(tab_RobotControl, text='X :',font=("Microsoft JhengHei", 12))
        word_label.place(x=400, y=160)


        validate_cmd = tab_RobotControl.register(self.validate_entry)  # 注册一个验证函数
        validate_input_cmd = tab_RobotControl.register(self.validate_input)  # 注册一个输入验证函数

        self.robot_draw_block_Entry_x = tk.Entry(tab_RobotControl, width=32, validate="all", validatecommand=(validate_cmd, '%P'), invalidcommand=(validate_input_cmd, '%P'),textvariable = self.robot_draw_block_String_x)
        self.robot_draw_block_Entry_x.place(x=430, y=165)

        word_label = tk.Label(tab_RobotControl, text='Y :',font=("Microsoft JhengHei", 12))
        word_label.place(x=700, y=160)

        self.robot_draw_block_Entry_y = tk.Entry(tab_RobotControl, width=32, validate="all", validatecommand=(validate_cmd, '%P'), invalidcommand=(validate_input_cmd, '%P'),textvariable = self.robot_draw_block_String_y)
        self.robot_draw_block_Entry_y.place(x=730, y=165)

        
        # 調色盤動作x12
        word_label = tk.Label(tab_RobotControl, text='色格測試 :',font=("Microsoft JhengHei", 12))
        ...
        for i in range(0,12):
            ...
            x_num =  int(i % 4)
            y_num =  int(i / 4)
            robot_pal_btn = tk.Button(tab_RobotControl, text=f'單色區塊 {i+1}', width=36, command=partial(self.common.RobotBehavior.fun_robot_behavior[7], i))
        ...
        # 調色盤主調色動作x4
        word_label = tk.Label(tab_RobotControl, text='調色測試 :',font=("Microsoft JhengHei", 12))
        ...
        for j in range(0,4):
            ...
            robot_pal_main_btn = tk.Button(tab_RobotControl, text=f'調色區塊 {j+1}', width=18, command=partial(self.common.RobotBehavior.fun_robot_behavior[8], j))
            robot_pal_main_btn.place(x=x_color + 150 * x_num, y=y_color + 50 * y_num)
            self.robot_pal_main_btns.append(robot_pal_main_btn)
        
        word_label = tk.Label(tab_RobotControl, text='主盤取色測試 :',font=("Microsoft JhengHei", 12))
        ...
        for j in range(0,4):
            ...
            robot_pal_main_btn = tk.Button(tab_RobotControl, text=f'調色區塊 {j+1}', width=18, command=partial(self.common.RobotBehavior.fun_robot_behavior[12], j))
            robot_pal_main_btn.place(x=x_color + 150 * x_num, y=y_color + 50 * y_num)
            self.robot_pal_main_btns.append(robot_pal_main_btn)
        
        word_label = tk.Label(tab_RobotControl, text='色盤是否符合目標顏色 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=820, y=370)
        
        robot_pal_main_btn = tk.Button(tab_RobotControl, text=f'判斷', width=18, command=partial(self.common.RobotBehavior.fun_robot_behavior[12], j))
        robot_pal_main_btn.place(x=1010, y=370)
        
        word_label = tk.Label(tab_RobotControl, text='第幾號 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=1180, y=370)
        
        num = tk.Entry(tab_RobotControl, width=9, validate="all", validatecommand=(validate_cmd, '%P'), invalidcommand=(validate_input_cmd, '%P'),textvariable = 'test')
        num.place(x=1250, y=375)
        
        word_label = tk.Label(tab_RobotControl, text='目標顏色 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=1180, y=370)
        
        # color_goal = tk.Entry(tab_RobotControl, width=9,textvariable = 'test')
        # color_goal.place(x=1220, y=370)
        
    def fun_tab_PicPC(self):
        
        
        word_label = tk.Label(self.tab_PicPC, text='線稿 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=10, y=5)
        
        self.picPc_ori_canvas = tk.Canvas(self.tab_PicPC, width=self.width_picPc_ori,
                                height=self.width_picPc_ori,bg='gray',highlightthickness=1, highlightbackground="black")
        self.picPc_ori_canvas.place(x=10, y=30)
    
        self.get_pic_ori_btn = tk.Button(self.tab_PicPC, text='線搞圖', width=36, command=self.get_pic_ori_draft)
        self.get_pic_ori_btn.place(x=10, y=590)
    
        self.get_pic_ori_bone_btn = tk.Button(self.tab_PicPC, text='查看線搞路徑', width=36, command=self.get_pic_ori_draft_bone)
        self.get_pic_ori_bone_btn.place(x=300, y=590)
    
        word_label = tk.Label(self.tab_PicPC, text='彩色上色圖 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=580, y=5)
    
        self.picPc_color_canvas = tk.Canvas(self.tab_PicPC, width=self.width_picPc_ori,
                                height=self.width_picPc_ori,bg='gray',highlightthickness=1, highlightbackground="black")
        self.picPc_color_canvas.place(x=580, y=30)
        
        self.get_pic_color_btn = tk.Button(self.tab_PicPC, text='彩色上色', width=36,command=self.get_pic_ori_colorful)
        self.get_pic_color_btn.place(x=580, y=590)
        
        word_label = tk.Label(self.tab_PicPC, text='分析後區塊',font=("Microsoft JhengHei", 12))
        word_label.place(x=1140, y=5)
        
        self.block_box_blocks = tk.Listbox(self.tab_PicPC, width=61, height=10,highlightthickness=1,exportselection=0)
        self.block_box_blocks.place(x=1140, y=30)
        
        self.block_box_blocks.bind("<<ListboxSelect>>", self.on_select_block)
        
        
        self.picPc_color_block_canvas  = tk.Canvas(self.tab_PicPC, width=250,
                                height=250,bg='gray',highlightthickness=1, highlightbackground="black")
        self.picPc_color_block_canvas.place(x=1140, y=210)
        
        self.colorful_box_blocks  = tk.Listbox(self.tab_PicPC, width=24,
                                height=25,highlightthickness=1,exportselection=0)
    ...
    def fun_tab_PicReal(self):
        ...
        word_label = tk.Label(self.tab_PicReal, text='實際繪圖 :',font=("Microsoft JhengHei", 12))
        word_label.place(x=10, y=5)
        
        self.picReal_canvas = tk.Canvas(self.tab_PicReal, width=self.width_picPc_ori,
                                height=self.width_picPc_ori,bg='gray',highlightthickness=1, highlightbackground="black")
        self.picReal_canvas.place(x=10, y=30)

        self.get_real_pic_btn = tk.Button(self.tab_PicReal, text='存檔', width=36, command=self.save_pic_real)
        self.get_real_pic_btn.place(x=10, y=590)
        
        self.get_real_pic_btn = tk.Button(self.tab_PicReal, text='比對繪畫結果', width=36, command=self.compare_pic_real)
    ...
    def fun_tab_ColorPalette(self):
        ...
        anay_para_setting_block = tk.Frame(self.tab_ColorPalette, 
                         width=840,
                         height=200,
        ...
        Entry_Start = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Pa_Start[0])
        ...
        Entry_End = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Pa_Start[1])
        ...
        Entry_Width = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Pa_Size[0])
        ...
        Entry_Heigh = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Pa_Size[1])
        ...
        Entry_Start = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Cell_Start[0])
        Entry_Start.place(x=x_init+20, y=y_init+75)
        
        label_01 = tk.Label(anay_para_setting_block, text='Y :')
        label_01.place(x=x_init+75, y=y_init+75)

        Entry_End = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Cell_Start[1])
        Entry_End.place(x=x_init+95, y=y_init+75)
        
        label_01 = tk.Label(anay_para_setting_block, text='W :')
        label_01.place(x=x_init+150, y=y_init+75)
        
        Entry_Width = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Cell_Size[0])
        Entry_Width.place(x=x_init+175, y=y_init+75)
        
        label_01 = tk.Label(anay_para_setting_block, text='H :')
        label_01.place(x=x_init+230, y=y_init+75)
        
        Entry_Heigh = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Cell_Size[1])
        ...
        Entry_Heigh = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Cell_Offset[0])
        ...
        Entry_Heigh = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Cell_Offset[1])
        ...
        Entry_Width = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Main_Size[0])
        Entry_Width.place(x=x_init+25, y=y_init+125)
        
        label_01 = tk.Label(anay_para_setting_block, text='H :')
        label_01.place(x=x_init+80, y=y_init+125)
        
        Entry_Heigh = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Main_Size[1])
        ...
        for i in range(0,len(self.common.Pal_main_start)):
            ...
            Entry_Start = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Main_Start[i][0])
            ...
            Entry_End = tk.Entry(anay_para_setting_block, width=6,textvariable = self.cam_Color_Main_Start[i][1])
            Entry_End.place(x=x_1, y=130)
        
        
        
        self.camreal_canvas = tk.Canvas(self.tab_ColorPalette, 
        ...
        anay_videos_block = tk.Frame(self.tab_ColorPalette, 
        ...
        for i in range(0,len(self.common.Color_Cells)):
          canvas = tk.Canvas(anay_videos_block, width=self.common.PalCell[0],
          ...
          r_x_start = 10+int(self.common.PalCell_offset[0]+60)*x_row+int(self.common.PalCell_start[0]) + x_row *10 + x_row * int(self.common.PalCell[0])
          r_y_start = 10+int(self.common.PalCell_offset[1]+10)*y_row+int(self.common.PalCell_start[1]) + y_row *10 + y_row * int(self.common.PalCell[1])
          
          label_01 = tk.Label(anay_videos_block, textvariable=self.cam_Color_Cell_info[i])
          label_01.place(x=r_x_start+28, y=r_y_start)
          
          label_01 = tk.Label(anay_videos_block, textvariable=self.cam_Color_Cell_HSV_info[i])
        ...
        # 計算 main 位置
        main_x_1_cal = 250+int(self.common.PalCell_offset[0]+10)*4+int(self.common.PalCell_start[0]) + 4 *10 + 4 * int(self.common.PalCell[0])
        main_x_2_cal = main_x_1_cal +int(self.common.PalCell_offset[0]+10)*6
        main_y_1_cal = 30+int(self.common.PalCell_start[1])
        main_y_2_cal = 5+int(self.common.PalCell_offset[1])*2+int(self.common.PalCell_start[1]) + 2 *10 + 2 * int(self.common.PalCell[1])
        ...
        for i in range(0,len(self.common.Color_Mains)):
            ...
            label_01 = tk.Label(anay_videos_block, textvariable=self.cam_Color_Main_Start_info[i])
            label_01.place(x=r_x_start+60, y=r_y_start)
            
            
            label_01 = tk.Label(anay_videos_block, textvariable=self.cam_Color_Main_Start_HSV_info[i])
            label_01.place(x=r_x_start+60, y=r_y_start+20)
            
            canvas = tk.Canvas(anay_videos_block, width=self.common.Pal_main[0],
        ...
        img_setting_block = tk.Frame(self.tab_ColorPalette, 
                         width=425,
                         height=220,
                         highlightthickness  = 1,
                         highlightbackground = 'black' )
        ...
        pal_contrast = tk.Entry(img_setting_block, width=6,textvariable = self.pal_contrast_string)
        pal_contrast.place(x=50, y=35)
        
        label_01 = tk.Label(img_setting_block, text='亮度 :')
        label_01.place(x=110, y=35)
        
        pal_brightness = tk.Entry(img_setting_block, width=6,textvariable = self.pal_brightness_string)
        ...
        pal_contrast = tk.Entry(img_setting_block, width=6,textvariable = self.main_color_contrast_string)
        pal_contrast.place(x=50, y=95)
        
        label_01 = tk.Label(img_setting_block, text='亮度 :')
        label_01.place(x=110, y=95)
        
        pal_brightness = tk.Entry(img_setting_block, width=6,textvariable = self.main_color_brightness_string)
    ...
    def updateWindow(self):
        ...
        while(self.common.updateWindow_bool):
            ...
            if self.photo_plant is not None :
                ...
                self.camreal_canvas.create_image(0, 0, image =self.photo_plant, anchor = tk.NW)
                self.camreal_canvas.config(width=width1, height=height1)
            if self.photo_real is not None :
                self.picReal_canvas.create_image(0, 0, image =self.photo_real, anchor = tk.NW)
            
            if len(self.common.Color_Cells) >0 :
                for i in range(0,len(self.common.Color_Cells)):
                    self.camreal_canvas_anyaize[i].create_image(0, 0, image =self.common.Color_Cells[i].Image, anchor = tk.NW)
            
            if len(self.common.Color_Mains) >0 :
                for i in range(0,len(self.common.Color_Mains)):
                    self.camreal_main_canvas_anyaize[i].create_image(0, 0, image =self.common.Color_Mains[i].Image, anchor = tk.NW)
            ...
            for i in range(0,len(self.common.Color_Mains)):
                if len(self.common.Color_Mains[i].HSV_Main) > 0 :
                    self.cam_Color_Main_Start_info[i].set(self.common.Color_Mains[i].Color_Pre)
                    hsv = f'({self.common.Color_Mains[i].HSV_Main[0]},{self.common.Color_Mains[i].HSV_Main[1]},{self.common.Color_Mains[i].HSV_Main[2]})'
                    self.cam_Color_Main_Start_HSV_info[i].set(hsv)
            
            for i in range(0,len(self.common.Color_Cells)):
                if len(self.common.Color_Cells[i].HSV_Main) > 0 :
                    self.cam_Color_Cell_info[i].set(self.common.Color_Cells[i].Color_Pre)
            ...
            self.common.main_color_brightness = int(self.main_color_brightness_string.get())
            
            self.common.get_ColorRealSize[0] = self.get_ColorRealSize[0].get()
            self.common.get_ColorRealSize[1] = self.get_ColorRealSize[1].get()
            self.common.get_ColorRealSize_start[0] = self.get_ColorRealSize_start[0].get()
            self.common.get_ColorRealSize_start[1] = self.get_ColorRealSize_start[1].get()
    ...
    def create_window(self):
        ...
        panel_tab = tk.Frame(self.window,width=self.window_width,height=self.window_height)
        panel_tab.grid(row=0, column=1, sticky="ns")
        
        tabsystem = ttk.Notebook(panel_tab)
        
        self.tab_RobotControl = tk.Frame(tabsystem,width=self.window_width,height=self.window_height)
        self.tab_PicPC = tk.Frame(tabsystem,width=self.window_width,height=self.window_height)
        self.tab_PicReal = tk.Frame(tabsystem,width=self.window_width,height=self.window_height)
        self.tab_ColorPalette = tk.Frame(tabsystem,width=self.window_width,height=self.window_height)
