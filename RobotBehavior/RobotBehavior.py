...
#         -沾顏料 不沾顏料
    ...
    def Thread_Active(self,active_function,pars=None):
        ...
        if pars == None:
            ...
            # pars 不做處理，直接帶入
            self.common.threading_robot_Move = threading.Thread(target = active_function,kwargs=pars)
    ...
    def Check_thread_alive(self):
        if self.common.threading_robot_Move is not None and self.common.threading_robot_Move.is_alive():
            print('尚未停止')
            return True
    ...
    def active_ColorClearTest(self):
        ...
        if self.common.status_robotTest_now != self.common.status_robotTest['Dig_Water']:
            self.common.status_robotTest_now = self.common.status_robotTest['Dig_Water']
            
            if last_status != self.common.status_robotTest['Dig_Water'] and last_status != self.common.status_robotTest['Dig_Issue'] :
    ...
    def Active_Water_Dip(self):
        ...
        wait =0.8
        self.mc.set_coords(self.common.border_DigWater[0], 40)
        time.sleep(wait)
        ...
        z = self.common.border_DigWater[2][2] + 8
    ...
    def active_ToiletDipTest(self):
        ...
        if self.common.status_robotTest_now != self.common.status_robotTest['Dig_Issue']:
    ...
    def Active_ColorDipTest(self,Dip_Num):
        ...
        self.Thread_Active(active_function = self.active_ColorDipTest,pars={"Dip_Num": Dip_Num})

    def active_ColorDipTest(self,Dip_Num):
        print(f'check----------------Active_ColorDip- {Dip_Num}')
        
        if self.common.status_robotTest_now != self.common.status_robotTest['Color_mixing']:
            self.common.status_robotTest_now = self.common.status_robotTest['Color_mixing']
            self.Active_ColorDip_center()
            
        self.Active_ColorDip(Dip_Num,border_clear = True)
        self.Thead_ForceStop()
        
    def Active_ColorDip(self,Dip_Num,border_clear = True):
        ...
        z2 = self.common.border_Palette[Dip_Num][2]
        
        
        x3 = self.common.border_Palette[Dip_Num][0]
        y3 = self.common.border_Palette[Dip_Num][1] - 0.5
        z3 = self.common.border_Palette[Dip_Num][2]
        
        x4 = self.common.border_Palette[Dip_Num][0]
        y4 = self.common.border_Palette[Dip_Num][1] - 2
    ...
    def Active_ColorMixingTest(self,Mixing_Num):
        ...
        self.Thread_Active(active_function = self.Active_ColorMixing,pars={"Mixing_Num": Mixing_Num})
    ...
    def Active_ColorMainTest(self,Mixing_Num):
        ...
        self.Active_ColorMixing(Mixing_Num)
        self.Thead_ForceStop()
    ...
    def Active_ColorMainTest(self,Mixing_Num):
        ...
        self.Thread_Active(active_function = self.active_ColorMainTest,pars={"Mixing_Num": Mixing_Num})

    def active_ColorMainTest(self,Mixing_Num):
        
        if self.common.status_robotTest_now != self.common.status_robotTest['Dig_Color']:
    ...
    def Active_ColorMixing(self,Mixing_Num):
        ...
        for i in range(0,8):
            ...
            #低2 (往右划)
            x_setp = self.common.border_Palette_Main[Mixing_Num][0] - (i+1) *x_once_step
    ...
    def Active_Color_MainDip(self,Mixing_Num):
        ...
        for i in range(0,1):
            ...
            x_setp = self.common.border_Palette_Main[Mixing_Num][0] - i *x_once_step
            y_setp = self.common.border_Palette_Main[Mixing_Num][1]+7.5
            if i % 2 == 1:
                y_setp = self.common.border_Palette_Main[Mixing_Num][1]-7.5
            ...
            y_setp = self.common.border_Palette_Main[Mixing_Num][1]+7
            if i % 2 == 1:
                y_setp = self.common.border_Palette_Main[Mixing_Num][1]-7
    ...
    def check_record_status(self,status:str):
        if self.common.record_robot_status != '' and self.common.record_robot_status == status:
            return True
    ...
    def Robot_Status_Change(self,status_robot):
        ...
        if self.common.status_robot_now == self.common.status_robot['None']:
            ...
            #不記錄record 免得影響pause
            self.Active_Paper_Dip()
        elif self.common.status_robot_now == self.common.status_robot['Color_Status_Check']:
            self.common.record_robot_status = 'Color_Status_Check'
        elif self.common.status_robot_now == self.common.status_robot['Color_Main_Select']:
            pass
        elif self.common.status_robot_now == self.common.status_robot['Color_Mix']:
            pass
        elif self.common.status_robot_now == self.common.status_robot['Color_Mix_Select_Check']:
            pass
    ...
    def active_ColorfulTest(self,i):
        ...
        if self.check_record_num(self.common.record_block_num,i)==False:
            return
        # print(f"測試開始")
        self.active_Colorful_Main_single(i)
        self.common.record_block_num = -1
        self.Thead_ForceStop()
    ...
    def active_Colorful_Main(self):
        ...
        if self.check_record_status('Color_Status_Check') and self.check_record_status('Pic_Draw') :
            
            for block_info_num in range(0,len(self.common.block_colorful_colors_info)):
                if self.check_record_num(self.common.record_block_num,block_info_num)==False:
                    continue
                
                self.active_Colorful_Main_single(block_info_num)

                # 整格block 內的顏色都判斷完並畫完後
                self.common.record_block_num = -1

    def active_Colorful_Main_single(self,block_info_num):
        
        # 檢查block每個顏色
        for colors_info_num in range(0,len(self.common.block_colorful_colors_info[block_info_num])) :
            if self.check_record_num(self.common.record_color_num,colors_info_num)==False:
                continue
            color_info = self.common.block_colorful_colors_info[block_info_num][colors_info_num]
            ...
            if self.check_record_status('Color_Status_Check'):
                ...
                #-2. 檢查顏色
                acitve_cmd,index,dis_layer =  self.active_ColorStatus_check(self.common.color_goal)
                ...
                if acitve_cmd =='Cells': #直接提取繪製
                    self.Active_Water_Dip_p_center()
                    self.Robot_Status_Change(self.common.status_robot['Water_Dip'])
                    self.Robot_Status_Change(self.common.status_robot['Color_Dip'])
                    ...
                    if self.active_ColorMix_Process(index,dis_layer):
                        ...
                        # 混色失敗-人工清理
                        return
                elif acitve_cmd =='Stop_and_ClearMain':  #進入人工停止清理模式後，再重新判斷
                    print(f"事前判斷失敗--人工清理")
                    return
            ...
            if self.check_record_status('Pic_Draw'):
                ...
                colorful_conturs = self.common.colorful_lines[block_info_num].groups_pens[colors_info_num]
                
                for colorful_lines_num in range(0,len(colorful_conturs)):
                    
                    if self.check_record_num(self.common.record_line_num,colorful_lines_num)==False:
                            continue
                    ...
                    for pen_num in  range(0,len(groups_pens)) :
                        ...
                        #start
                        self.mc.set_coords(pen.point_start, 60)
                        time.sleep(1)
                        
                        # 繪圖過程
                        for one_point in pen.point_precess:
                            self.mc.set_coords(one_point, 40)
                            time.sleep(0.7)
                        
                        #end
                        self.mc.set_coords(pen.point_end, 60)
                        time.sleep(1)
    ...
    def active_ColorStatus_check(self,color_goal):
        ...
        for main_num in range(0,len(self.common.Color_Mains)):
                ...
                if self.common.Color_Mains[main_num].Color_Pre != "白色" :
                    ...
                    # TODO 錯誤!! 如果主盤是最左或最右的色，會找錯色系!

                    goal_system_num,goal_system_pos=self.get_system_color_num_pos(color_goal)
                    main_system_num,main_system_pos=self.get_system_color_num_pos_match_goal(self.common.Color_Mains[main_num].Color_Pre,color_goal)
                    
                    if goal_system_num != -1 and main_system_num !=-1 and goal_system_num == main_system_num:
    ...
    def get_system_color_num_pos(self,color_name):
        ...
        for color_system_num in range(0,len(self.common.Systems_Colors)):
            if color_name in self.common.Systems_Colors[color_system_num]:
                return color_system_num,self.common.Systems_Colors[color_system_num].index(color_name)
        ...
        for color_system_num in range(0,len(self.common.Systems_Colors)):
            if color_name in self.common.Systems_Colors[color_system_num]:
                get_color_sys_pos.append(
                {
                 ...
                 'sys_num_pos':self.common.Systems_Colors[color_system_num].index(color_name)
                 })
    ...
    def active_ColorMix_Process(self,main_num,dis):
        ...
        while(isMatch_goal == False):
           ...
           #哪一格
           print(f'混色---目標顏色:{self.common.color_goal},色系為:{system_num},取顏色:{color_name}')
           ...
           for cell_num in range(0,len(self.common.Color_Cells)):
              if self.common.Color_Cells[cell_num].Color_Pre == color_name:
                  ...
                  self.Active_ColorDip(cell_num)
                  is_find_cell_color = True
                  break
           
           if is_find_cell_color == True:
              #主盤調色
              self.Active_ColorMixing(main_num)
            
           #避免擋住攝影機
           self.Active_Water_Dip_p_center()
           time.sleep(0.5)
           # 再次檢查 
           isMatch_goal,goal_system_num,goal_system_pos,main_system_num,main_system_pos = self.isMatch_goal_color(main_num,self.common.color_goal)
        ...
        for p in draft_path :
            ...
            for i in range(0,count_pans):
                ...
                for j in range(num_min,num_max+1):
                    ...
                    range_robot=y * abs(self.common.z_range[0] -self.common.z_range[1])
                ...
                for color_coutur in color_couturs :
                    for path in color_coutur :
                        ...
                        for p_n in range(0,len(path)) :
                            ...
                            p_min = [float(path[num_min][0] /width), float(path[num_min][1]/height)]
                            p_max = [float(path[num_max][0] /width), float(path[num_max][1]/height)]
                            ...
                            for j in range(num_min,num_max+1):
                                p_pro = [float(path[j][0] /width), float(path[j][1]/height)]
                                p_process   =  self.Active_MainTranlation(p_pro[0],p_pro[1])
